...
import { getExpensesDashboard, exportExpensesReport } from '../../services/api';

export default function ExpensesPage() {
  const { data, loading, error, filters, applyFilters } = useReportDashboard(getExpensesDashboard);
    ...
    <Box sx={{ p: 4 }}>
      <Stack spacing={3}>
        <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ md: 'center' }} spacing={2}>
          ...
          <ExportButtons onExport={(format) => exportExpensesReport(format, filters)} />
        </Stack>
        ...
        {data ? (
          <>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <ChartCard title="Expense Trend" type="line" data={data.trend} />
              </Grid>
              <Grid item xs={12}>
                <ChartCard
